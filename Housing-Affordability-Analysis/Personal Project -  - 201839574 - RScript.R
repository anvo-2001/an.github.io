...
setwd("C:/Users/PC - MSI/OneDrive - The University of Liverpool/An - 2025/2024 - UoL/ECON 705 Data Mgt/Exercise/Assignment1/Assignment1_Data")
library(readxl)
library(writexl)
library(tidyverse)
library(ggplot2)
...
region_data_cleaned[region_data_cleaned$Region_Name == 'England',] %>%
  ...
  geom_point(data = subset(region_data_cleaned, Region_Name == 'England' & Year %in% c(2008, 2021)), 
             aes(x = Year, y = Median_House_Price / 1000), color = "darkblue", size = 3) +
  geom_point(data = subset(region_data_cleaned, Region_Name == 'England' & Year %in% c(2008, 2021)), 
             aes(x = Year, y = 5 * Median_Earnings / 1000), color = "#56B4E9", size = 3) +
  geom_text(data = subset(region_data_cleaned, Region_Name == 'England' & Year %in% c(2008, 2021)),
