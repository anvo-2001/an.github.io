...
order by 1, 2 desc
	...
	-- The number is adjusted to population of 1,000,000. This allows fair comparisons between countries of different population sizes.
	-- Using CTE
with smoothed as
(
select location, date, population
	, ISNULL(new_cases,0) as new_cases
	, (SUM(CAST(new_cases as float)) 
	over(partition by location order by date asc rows between 6 preceding and current row))/7 as new_cases_smoothed
	, ISNULL(new_deaths,0) as new_deaths
	, (SUM(CAST(new_deaths as float))
	over(partition by location order by date asc rows between 6 preceding and current row))/7 as new_deaths_smoothed
from #CD
)
select *
	, ISNULL(CAST((new_cases_ma/population*1000000) as decimal(10,3)),0) as new_cases_per_million
	, ISNULL(CAST((new_deaths_ma/population*1000000) as decimal(10,3)),0) as new_deaths_per_million
from smoothed
...
select continent, location, date, population
	...
	, SUM(new_cases) over (partition by location order by date asc) as cummulative_cases
	, SUM(new_deaths) over (partition by location order by date asc) as cummulative_deaths
	...
	, ISNULL((d10_cases/NULLIF(new_deaths_smoothed,0)),0) as case_fatality_ratio
from lag_10d
