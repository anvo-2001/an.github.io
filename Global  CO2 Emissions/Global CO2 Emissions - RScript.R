...
setwd("C:/Users/PC - MSI/OneDrive - The University of Liverpool/An - 2025/2024 - UoL/ECON 705 Data Mgt/Exercise/FINAL")
library(readxl)
library(writexl)
library(tidyverse)
library(ggplot2)
...
main_data <- data_merged %>% 
  ...
  mutate(CO2_emissions = CO2_emissions / 1e9)
...
ggplot(world_data, aes(x = Year)) +
  ...
  geom_line(aes(y = CO2_emissions), color = "skyblue", size = 1.5) +
  ...
  scale_y_continuous(breaks = seq(0, 40, by = 5),
    name = "CO2 Emissions (billion tonnes)",
    sec.axis = sec_axis(~ ., name = "% Change in Emissions (0-20%)")
  ) +
  # Labels and customization
  labs(
    title = "Annual CO2 Emissions and Yearly Changes (1950-2022, World)",
    x = "Year"
  ) +
  theme_minimal() +
  theme(
...
regions <- c(
  ...
  "Asia (excl. China and India)" = "#6A5ACD",  # Soft indigo for distinctiveness
  ...
  "Africa" = "#32CD32",  # Lime green for energy
...
data2 <- main_data %>% filter(Year==2022) %>%
  ...
  select(Entity,CO2_emissions)%>%
  head(5)
# Calculate Rest of the World emissions
total_emissions <- data_2022$CO2_emissions[data_2022$Entity == "World"]
rest_of_world_emissions <- total_emissions - sum(data2$CO2_emissions)
data2 <- data2 %>%
  add_row(
    Entity = "Rest of the World",
    CO2_emissions = rest_of_world_emissions) %>%
  arrange(desc(CO2_emissions)) %>%
  mutate(Percentage_of_Total = (CO2_emissions / total_emissions) * 100,
    Label = paste0(round(CO2_emissions, 2), " (", round(Percentage_of_Total, 1), "%)"),
    BarColor = c("#FFAA00","darkgrey", "#6A5ACD","#4682B4","#FF6347", "darkblue")
  )
# Create the bar chart
ggplot(data2, aes(x = reorder(Entity, CO2_emissions), y = CO2_emissions, fill = BarColor)) +
  # Horizontal bars
...
data4 <- main_data %>%
  ...
  select(Entity, Code, Year, CO2_emissions, CO2_emissions_capita,Pct_change_emissions) %>%
  group_by(Entity) %>%  # Group by country
  arrange(Year) %>%  # Ensure data is sorted by year for cumulative calculation
  mutate(Cumulative_CO2_emissions = cumsum(CO2_emissions)) %>%  # Calculate cumulative sum
  ungroup() 
...
# FIGURE 5
data5 <- rawdata_continents %>% 
  rename(iso_a3 = Code) %>% 
  left_join(data_2022, by = c("Entity", "iso_a3")) %>%
  filter(Year==2022, !is.na(GDP_capita), !is.na(CO2_emissions_capita)) %>%
  select (iso_a3,Continent,GDP_capita, CO2_emissions_capita,Population)

# Create a custom color mapping for continents
continent_colors <- c(
  "Asia" = "#6A5ACD",
  "Africa" = "#32CD32",
  "North America" = "#990000",
...
ggplot(data5, aes(x = GDP_capita/1000 , y = CO2_emissions_capita, size = Population / 1e6, color = Continent)) +
  geom_smooth(method = "lm", se = FALSE, color = "#78909C", linewidth = 0.5) +  # Add regression line
  geom_point(alpha = 0.7) +  # Use bubbles with transparency
  geom_text_repel(aes(label = iso_a3), size = 3, max.overlaps = 10) +  # Add country codes as labels
  labs(
    title = "Scatter Plot of CO₂ Emissions vs GDP per Capita by Continent (2022)",
    x = "GDP per Capita (thousands $)",
    y = "CO2 Emissions per capita ( billion tons)",
    size = "Population (millions)",
    color = "Continent"
  ) +
  theme_minimal() +  # Minimal theme for a clean look
  scale_color_manual(values = continent_colors) +  # Apply custom colors for continents
  theme(
...
## Regression summary
lm_model <- lm(CO2_emissions_capita~GDP_capita, data=data5)
summary(lm_model)
