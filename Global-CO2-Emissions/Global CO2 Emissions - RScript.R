...
data5 <- rawdata_continents %>% 
  left_join(data_2022, by = c("Entity", "Code")) %>%
  filter(Year==2022, !is.na(GDP_capita), !is.na(CO2_emissions_capita)) %>%
  mutate(GDP = GDP_capita*Population, CO2_emissions = CO2_emissions_capita*Population
         ,lgdp = log(GDP)
...
ggplot(data5, aes(x = GDP_capita_thousand , y = CO2_emissions_capita, size = Population / 1e6, color = Continent)) +
  ...
  geom_text_repel(aes(label = Code), size = 3, max.overlaps = 10,segment.color = NA) +
  labs(
...
## Regression summary
lm_model <- lm(lco2~lgdp, data=data5)
summary(lm_model)

data_2021 <- main_data %>% rename  filter(Year == 2021 & (!is.na(Code))) %>%
  mutate(GDP_capita_thousand = GDP_capita/1000)
lm_model1 <- lm(Consumption_CO2_emissions_capita~GDP_capita_thousand, data=data_2021)
summary(lm_model1)
